...
class GenMuonAnalyzer : public edm::EDAnalyzer {
  ...
  TH1F * h1_status;;
  TH1F * h1_nMu, * h1_nMuP, * h1_nMuN, * h1_MuMuMass;
  TTree * tree;
  int run_, evtnum_;
  double genweight_;
};
...
GenMuonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if ( nMuP ==1 && nMuN == 1 ) {
    ...
    h1_MuMuMass->Fill(lv_MuMu.mass(),genEvt_weight);
  }
...
//  h1_MuMuMass = fs->make<TH1F>( "h1_MuMuMass"  , "h1_MuMuMass", 1001, 71.1888000,111.190000);
  h1_MuMuMass = fs->make<TH1F>( "h1_MuMuMass"  , "h1_MuMuMass", 1001, 71.17,111.21);
