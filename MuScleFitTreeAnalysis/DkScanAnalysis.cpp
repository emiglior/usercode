...
static const double up_limit = 1./100.;
static const int binning = 40;

DkScanAnalysis::DkScanAnalysis(TFile * fout, const char * append) :
  HList_pos(0), HList_neg(0), Canvas(0), dk(0), chi2(0), ks(0) {

  sprintf(dirname,"./mc_pt_%i_test/",(int)pt_lep);
  mkdir(dirname,0775);
  ...
  if ( fout != 0 ) {
    ...
    char dir_title[40];
    sprintf(dir_title,"DkScanAnalysis%s",append);
    ...
    TH1F * h_pos;
    TH1F * h_neg; // create the pointers to the histograms
    TCanvas * c_temp;
    for (int i = 0; i < n_Dk; i++) {
      ...
      h_pos->SetTitle("Leading muon curvature q/p_{T};#kappa [c/GeV];Entries");
      h_neg->SetTitle("Leading muon curvature q/p_{T};#kappa [c/GeV];Entries");
      HList_pos.Add(h_pos);
      HList_neg.Add(h_neg);
...
void DkScanAnalysis::analyze(const TLorentzVector & muNeg, const TLorentzVector & muPos, double weight){

  the_dir->cd();
  float delta_kappa=-0.0005;//-0.0005;//0.0005;//0.001;
  float k_prime;
  ...
  for(int j=0;j<n_Dk;j++){
    ...
    if(k_prime >0){
      ((TH1F*)h_pos_temp)->Fill(k_prime);
    }
    else{
      ((TH1F*)h_neg_temp)->Fill(k_prime*(-1)); //filling with the negative to have the same histogram range for the chi2 comparison
    }	
