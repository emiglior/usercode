void copyTreeWithWeights(){
  ...
  // then create a uniquw TTree with: hadd GenMuonAnalyzerHistos_PFS_afterFSRv2_POWHEG_50_1400_NoCuts_Weighted.root powheg_M50_120.root powheg_M120_200.root powheg_M200_400.root powheg_M400_800.root powheg_M800_1400.root
  ...
  char oldfile_name[nsample][70] = {
    ...
    "GenMuonAnalyzerHistos_PFS_afterFSRv2_POWHEG_800_1400_NoCuts.root"    
  };
  ...
  Double_t new_weights[nsample] = {
    ...
    lumi*0.241*1.006/80764,
    lumi*0.01678*1.006/63002
  };
  ...
  for (Int_t is=0; is<nsample; is++) {                        // loop on the samples
    ...
    for (Long64_t i=0;i<nentries; i++) {
      ...
      Bool_t EtaAcceptance = 
	( TMath::Abs((*muNegGen).Eta()) < 2.4 ) &&
	( TMath::Abs((*muPosGen).Eta()) < 2.4);
      
      Double_t mLL = ((*muNegGen)+(*muPosGen)).M();
      if ( EtaAcceptance && mLL >= min_mLL[is]  &&  mLL <= max_mLL[is] ) 
       	newtree->Fill();
